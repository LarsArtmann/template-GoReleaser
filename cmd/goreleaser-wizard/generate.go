...
func runGenerate(cmd *cobra.Command, args []string) {
	...
	if config.ProjectName == "" {
		...
		if config.ProjectName == "" {
			fmt.Println(errorStyle.Render("✗ Project name is required (use --name)"))
			os.Exit(1)
		}
	}
	...
	if !force && fileExists(".goreleaser.yaml") {
		fmt.Println(errorStyle.Render("✗ .goreleaser.yaml already exists (use --force to overwrite)"))
		os.Exit(1)
	}
...
{{end}}`
	...
	file, err := os.Create(".goreleaser.yaml")
	if err != nil {
		return err
	}
	defer file.Close()
...
`
	...
	file, err := os.Create(filepath.Join(".github", "workflows", "release.yml"))
	if err != nil {
		return err
	}
	defer file.Close()
